...
void KHCore::casestudy(string core_path, string authors ,string name)
{
	int n_core;
	FILE *fin = fopen(core_path.c_str(), "rb");
	if( fin == NULL ) return;
	fread(&n_core, sizeof(int), 1, fin);
	if (n_core != n){
		printf("n!=n_core\n");
		abort();
	}
	vector<int> core; core.reserve(n_core);
	...
	int x = -1, a;
	char Buff[1024];
	vector< string > ent_author(n, "");
	while(fgets(Buff, 1024, in) != NULL){
		if (x == -1 && strstr(Buff, name.c_str()) != NULL){
			sscanf(Buff, "%d", &x);
			//break;
		}
		sscanf(Buff, "%d", &a);
	...
	fclose(in);
	if (x < 0){
		printf("Not find the professor:%s\n", name.c_str());
		abort();
	}
	printf("%s = %d, core num=%d, d=%d\n", name.c_str(), x , core[x], deg[x]);

	vector<bool> visited(n, false);
	vector<int> q; q.push_back(x);
	vector< pair<int,int> >eg;
	for (size_t i = 0, cnt = 0; i < q.size() && cnt < 1; ++i){
		int v = q[i];
	...
	for (size_t i = 0; i < q.size(); ++i){
		...
		for (int j = 0; j < deg[v]; ++j){
			int u = adj[v][j];
			if (visited[u] && v < u)
				eg.push_back(make_pair(v, u));
		}
	}

	printf("nums=%ld\n", q.size());
	for (size_t i = 0; i < q.size();++i)
		printf("Author[%d]=%s\n",q[i], ent_author[q[i]].c_str());
	sprintf(Buff,"casestudy-edges-%s-num-%ld.txt", str.c_str(), q.size());
